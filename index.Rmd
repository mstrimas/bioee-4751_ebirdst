...
To get started with this lab, open up RStudio and create a new project from the File menu. This will create a new directory on your commputer in which will contain your code and data for this project. Create a new R script (File > New File) to contain all the code in this lab and save it in your project directory. Finally, download [this file of GIS data](https://github.com/mstrimas/bioee-4751_ebirdst/raw/master/data/gis-data.gpkg), which contains some contextual gis layers that we'll use when making maps. Place this file in a `data/` subdirectory of your project.
...
# extract weeekly occurrence and abundance values
...
```{r seasonal-prep}
# project layers
abd_seasonal_proj <- abd_seasonal[[c("breeding", "nonbreeding")]] %>% 
  crop(map_pars$fa_extent_sinu + 2e6) %>% 
  projectRaster(crs = proj, method = "ngb")
...
The above weekly mapping code can be repurposed to produce seasonal abundance maps.
